...
func NewRouterV2(db *pgxpool.Pool, origins []string) http.Handler {
	api := &API{db: db}
...
func (api *API) userAccess(r *http.Request, planID, email string) (string, bool, error) {
	...
	err := api.db.QueryRow(r.Context(), `
		SELECT u.id,p.created_by=u.id FROM users u JOIN plans p ON p.id=$1
		WHERE u.email=$2 AND (p.created_by=u.id OR p.group_id IN
		(SELECT group_id FROM group_members WHERE user_id=u.id))`, planID, email).Scan(&userID, &owner)
	return userID, owner, err
}

func (api *API) getPlanDetail(w http.ResponseWriter, r *http.Request) {
	planID := r.PathValue("id")
	userID, owner, err := api.userAccess(r, planID, r.URL.Query().Get("email"))
	...
	var locationName, address *string
	if err := api.db.QueryRow(r.Context(), `SELECT title,description,type,status,confirmed_date,location_name,address FROM plans WHERE id=$1`, planID).Scan(
		&title, &description, &planType, &status, &confirmedDate, &locationName, &address,
	); err != nil {
		writeError(w, http.StatusInternalServerError, "could not load plan")
		return
	}
	...
	writeJSON(w, http.StatusOK, map[string]any{
		"id": planID, "title": title, "description": description, "type": planType, "status": status,
		"confirmed_date": confirmedDate, "location_name": locationName, "address": address,
...
func (api *API) updatePlan(w http.ResponseWriter, r *http.Request) {
	...
	_, err = api.db.Exec(r.Context(), `UPDATE plans SET title=$1,description=$2,location_name=NULLIF($3,''),address=NULLIF($4,''),confirmed_date=COALESCE($5,confirmed_date) WHERE id=$6`, input.Title, strings.TrimSpace(input.Description), strings.TrimSpace(input.LocationName), strings.TrimSpace(input.Address), confirmed, r.PathValue("id"))
	if err != nil {
...
func (api *API) deletePlan(w http.ResponseWriter, r *http.Request) {
	_, owner, err := api.userAccess(r, r.PathValue("id"), r.URL.Query().Get("email"))
	if err != nil || !owner {
...
func (api *API) voteRegisteredPlan(w http.ResponseWriter, r *http.Request) {
	...
	for optionID, vote := range input.Votes {
		...
		result, err := tx.Exec(r.Context(), `INSERT INTO plan_date_votes(option_id,user_id,vote)
...
func (api *API) loadTasks(r *http.Request, planID, userID string) ([]planTask, error) {
	rows, err := api.db.Query(r.Context(), `SELECT t.id,t.title,t.status,COALESCE(u.name,t.assigned_to_guest_name),COALESCE(t.assigned_to_user_id=$2,false)
...
func (api *API) createTask(w http.ResponseWriter, r *http.Request) {
	...
	var taskID string
	if err := api.db.QueryRow(r.Context(), `INSERT INTO tasks(plan_id,title,assigned_to_user_id) VALUES($1,$2,$3) RETURNING id`, r.PathValue("id"), input.Title, userID).Scan(&taskID); err != nil {
